...
- **Content:** the blog is `@nuxt/content`-driven. `content.config.ts` defines a `blog` collection (`type: "page"`, source `blog/**.md`) with a Zod schema (`date`, `title`, `description`, `tags`, `read`, `future`); the posts live in `content/blog/*.md` and the blog pages query them live via `queryCollection("blog")`. Everything else (projects, cv, domains, goodies, social, im, command palette) stays as hardcoded TypeScript modules in `app/data/*.ts` - there is no `app/data/posts.ts`.
