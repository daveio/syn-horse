...
**Do not modify** the `nitro.cloudflare.wrangler` block (custom domains, bindings, account id, dev host). `worker-configuration.d.ts` is auto-generated by `wrangler types` as part of `bun run build` - never hand-edit it.
...
- `bun run deploy` - `wrangler deploy` to production. **Do not run without explicit user request.**
- Run `wrangler types` (or `bun run build`) after changing bindings.
